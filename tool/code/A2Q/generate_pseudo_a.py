...
# 本地模型软链接路径（你已建立的）
MODEL_PATH_Q2A = os.path.expanduser("/public/home/robertchen/zh20254227049/code/project/models/qwen3Lora_combine_Q2A_2")
    ...
    return [
        {
            ...
            "content": "You are an expert answer generator. "
                       ...
                       "Requirements: "
                       "- The answer must be in fluent Chinese "
...
def build_qa_pairs(segments):
    tok_a2q, mdl_a2q = load_model(MODEL_PATH_Q2A)
    pairs = []
    for s in segments:
      q = s['question']
      a_gen = chat_generate(
      ...
      temperature=0.0,
      enable_thinking=False)
      pairs.append({"question": q, "answer": a_gen})
    return pairs
...
if __name__ == "__main__":
  ...
  for item in qa_pairs:
    ...
    converted.append({
        "instruction": (
            "请根据以下【答案】生成一个自然且不含歧义的中文问题，"
            "使得该问题的唯一合理回答就是该【答案】。\n"
            "要求：至少15个中文字符，包含重要实体、原因或比较，避免是/否问句。"
        ),
      "input": a,           # 原问题没有额外输入，可留空
      "output": q,
      "system": "",          # 如果无系统提示可留空
      "history": []          # 若是多轮对话，这里可填[[q1,a1],[q2,a2]]
    })
  output_path = "/public/home/robertchen/zh20254227049/code/project/LLaMA-Factory/data/A2Q_pseudo_question_2.json"
